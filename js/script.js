...
function renderPassport() {
  ...
  if (risk) {
    ...
    risk.style.background = 'none';
    risk.style.color = p.riskColor;
  }
  ...
  if (statsEl && p.stats) {
    const statsIconMap = {
      'Total Transaksi': { icon: 'fa-receipt', color: 'blue' },
      'Rata-rata/Hari': { icon: 'fa-clock', color: 'blue' },
      'Produk Terlaris': { icon: 'fa-star', color: 'green' },
      'Rating Pelanggan': { icon: 'fa-user', color: 'orange' }
    };
    statsEl.innerHTML = p.stats.map(s => {
      const m = statsIconMap[s.label] || { icon: 'fa-chart-simple', color: 'blue' };
      return `
      <div class="passport-detail-card">
        <div class="pdc-icon ${m.color}"><i class="fas ${m.icon}"></i></div>
        <div class="pdc-info">
          <span class="pdc-label">${s.label}</span>
          <span class="pdc-value">${s.value}</span>
        </div>
      </div>`;
    }).join('');
  }
...
function handleBusinessProfile(e) {
  ...
  var profileData = {
    ...
    age: document.getElementById('businessAge').value,
    revenue: document.getElementById('monthlyRevenue').value,
    employees: document.getElementById('employeeCount').value,
    hours: document.getElementById('operationalHours').value
  };
  localStorage.setItem('aksaBusinessProfile', JSON.stringify(profileData));
...
function initSidebar() {
  ...
  sidebarConfig.items.forEach(item => {
    ...
    const logoutAttrs = isLogout
      ? ` onclick="localStorage.removeItem('aksaLoggedIn');localStorage.removeItem('aksaOnboarded');localStorage.removeItem('aksaSyncTime');localStorage.removeItem('aksaDataSource');localStorage.removeItem('aksaManualData');localStorage.removeItem('aksaTransactionData');localStorage.removeItem('aksaBusinessProfile');window.location.href='${item.href}'"`
      : ` href="${item.href}"`;
    html += `
...
function generatePassportPDF() {
  fetch('passportpdf.html')
    ...
    .then(function(html) {
      ...
      var container = doc.querySelector('.passport-container');
      if (!styleTag || !container) return;

      var wrapper = document.createElement('div');
      wrapper.style.cssText = 'position:absolute;top:-9999px;left:0;width:210mm;background:#fff;';
      ...
      var opt = {
        margin: [10, 0, 10, 0],
        filename: 'business-passport.pdf',
        image: { type: 'jpeg', quality: 0.98 },
        html2canvas: { scale: 2, useCORS: true, width: wrapper.scrollWidth, windowWidth: wrapper.scrollWidth },
